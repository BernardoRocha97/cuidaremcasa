...
// Diagnósticos e intervenções de enfermagem (terminologia clínica)

const nursingCatalogSchema = z.object({
  name: z.string().min(1, "Nome obrigatório"),
  description: z.string().optional(),
});

function parseNursingCatalogForm(formData: FormData) {
  return nursingCatalogSchema.parse({
    name: formData.get("name"),
    description: (formData.get("description") as string) || undefined,
  });
}

export async function createNursingDiagnosis(formData: FormData) {
  await requireAdmin();
  const data = parseNursingCatalogForm(formData);
  await prisma.nursingDiagnosis.create({ data });
  revalidatePath("/catalogo");
  redirect("/catalogo");
}

export async function updateNursingDiagnosis(id: string, formData: FormData) {
  await requireAdmin();
  const data = parseNursingCatalogForm(formData);
  await prisma.nursingDiagnosis.update({ where: { id }, data });
  revalidatePath("/catalogo");
  revalidatePath(`/catalogo/diagnosticos/${id}`);
}

export async function setNursingDiagnosisActive(id: string, active: boolean) {
  await requireAdmin();
  await prisma.nursingDiagnosis.update({ where: { id }, data: { active } });
  revalidatePath("/catalogo");
  revalidatePath(`/catalogo/diagnosticos/${id}`);
}

export async function createNursingIntervention(formData: FormData) {
  await requireAdmin();
  const data = parseNursingCatalogForm(formData);
  await prisma.nursingIntervention.create({ data });
  revalidatePath("/catalogo");
  redirect("/catalogo");
}

export async function updateNursingIntervention(id: string, formData: FormData) {
  await requireAdmin();
  const data = parseNursingCatalogForm(formData);
  await prisma.nursingIntervention.update({ where: { id }, data });
  revalidatePath("/catalogo");
  revalidatePath(`/catalogo/intervencoes-enfermagem/${id}`);
}

export async function setNursingInterventionActive(id: string, active: boolean) {
  await requireAdmin();
  await prisma.nursingIntervention.update({ where: { id }, data: { active } });
  revalidatePath("/catalogo");
  revalidatePath(`/catalogo/intervencoes-enfermagem/${id}`);
}
